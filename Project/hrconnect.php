...
function alert($x){
	echo"<script>alert('$x');
	window.location.href='hrregister.php';
	</script>";
	}
...
$email=$_POST['email'];
$s="select * from hrreg where username ='$username'";
...
if($num==1){
	...
	$reg= "insert into hrreg(username,password,email) values('$username','$password','$email')";
